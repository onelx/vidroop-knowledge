...
2. **El crawler recorre las 24 rutas estáticas + el storefront público** (`/tienda` → tienda.vidroop.com/academia/{slug}) **+ auto-descubrimiento de entidades dinámicas (2026-05-30, punto (a) — implementado, SIN verificar end-to-end)**. `discoverDynamicTargets()` en `crawler/src/index.ts` visita `/formaciones` y `/productos`, extrae los UUIDs por path-scan del HTML (`/curso/{uuid}`, `/mapa/{uuid}`, `/producto/{uuid}`) con fallback a sniff de respuestas JSON de api.vidroop.com (sólo si el path-scan da 0), y expande cada entidad en sus sub-páginas (`CURSO_SUBJECTS`/`PRODUCTO_SUBJECTS`/`MAPA_SUBJECTS` en `crawler/src/routes.ts`). Reusa `capturePage` + detección de cambios. **BLOQUEANTE para verificar: la academia tiene 0 cursos/productos** — hay que crear ≥1 curso y ≥1 producto en Vidroop y luego correr un crawl. El trial de Vidroop vence ~2026-06-03. El detalle detrás de modales/forms (Crear X) sigue necesitando el explorer agent (Etapa F).
...
- **(a) Auto-descubrir cursos/productos** → **CÓDIGO IMPLEMENTADO (2026-05-30), pendiente de verificar.** `discoverDynamicTargets()` lee `/formaciones` y `/productos`, saca los UUIDs (path-scan del HTML + fallback API) y expande las sub-páginas (`/curso/{id}/{tarjeta,contenidos,activacion,alumnos/alumnos,alumnos/invitaciones,alumnos/estadisticas,mensajes}`, las 9 de `/producto/{id}/...` y un set conservador de `/mapa/{id}/...`). Compila (`cd crawler && npm run build`). **Falta verificar end-to-end: crear ≥1 curso y ≥1 producto en Vidroop, correr un crawl y confirmar que aparecen las sub-páginas en `paginas`.** Sin commit/push todavía.
